...
<head>
    ...
    <title>ADMIN | TODAYS BOOKINGS</title>
                    ...
                    <div class="stage-body">
                        <table class="kenolykovitable dataTable table table-striped table-hover">
                            ...
                            <tbody>
                                ...
                                $t1="23:59:59";
                                $to=date('Y-m-d')." ".$t1;
                                $todays_booking="SELECT students.fullname,students.regNo, students.contactno,hostel.hostelName,room.roomname,studentroombook.bookingDate,studentroombook.id from studentroombook join students on  studentroombook.registrationNo=students.regNo join room on room.id=studentroombook.bookedroomid join hostel on room.hostelid=hostel.id WHERE studentroombook.bookingDate BETWEEN ? and ?  ";
                                $serial=0;
                                $student_today_booked=$con->prepare($todays_booking);
                                $student_today_booked->bind_param('ss',$from,$to);
                                $student_today_booked->execute();
                                $students=$student_today_booked->get_result();
                                while($Rows=$students->fetch_assoc()){
                                    ...
                                    <tr>
                                        ...
                                        <td><a href="student-update-room-booking.php?bkid=<?php echo htmlentities($booking_statusid);?>" title="Update Payment Status" target="_blank"><i class=" fa fa-user-alt"></i></a></td>
                                    </tr>
                                <?php } ?>
                            </tbody>
                        </table>
